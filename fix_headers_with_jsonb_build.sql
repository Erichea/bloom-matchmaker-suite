-- Fix headers by using jsonb_build_object instead of format string
-- This avoids any potential escaping issues with the service key
...
BEGIN
  ...
  RAISE LOG 'Service key retrieved, length: %', LENGTH(v_service_role_key);
  ...
  -- Build params with headers using jsonb_build_object (safer than format string)
  ...
  RAISE LOG 'Calling edge function: %', v_url;
  RAISE LOG 'With body: %', v_body;
  RAISE LOG 'With params: %', v_params;
