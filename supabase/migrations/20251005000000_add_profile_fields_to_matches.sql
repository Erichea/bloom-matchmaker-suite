  ...
  SELECT json_build_object(
      ...
      'profile_1', json_build_object(
        ...
        'photo_url', (
          ...
          ORDER BY is_primary DESC NULLS LAST, order_index ASC NULLS LAST, created_at ASC NULLS LAST
          LIMIT 1
        ),
        'photos', COALESCE((
          ...
          ) ORDER BY pp.is_primary DESC NULLS LAST, pp.order_index ASC NULLS LAST, pp.created_at ASC NULLS LAST)
          FROM (
            SELECT id, photo_url, is_primary, order_index, created_at
            FROM profile_photos
            WHERE profile_id = p1.id
            ORDER BY is_primary DESC NULLS LAST, order_index ASC NULLS LAST, created_at ASC NULLS LAST
            LIMIT 6
