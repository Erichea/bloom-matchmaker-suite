-- Fix pg_net authorization header format for edge function calls
-- The issue was that jsonb_build_object was not formatting the Bearer token correctly
...
BEGIN
  ...
  -- Call edge function with properly formatted authorization header
  -- pg_net.http_post expects: url, body, params (headers go in params as jsonb)
  BEGIN
